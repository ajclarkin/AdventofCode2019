...
# Modified version of interactive.py
# To use need to instatiate and then call RunIntocode() method
# One-to-one relationship between input and output so ops 3 and 4 altered for that
...
class Intcode:
    ...
    def RunIntcode(self, inputdata = None):
        # if inputdata contains an int that becomes the input value for the next op 3
        # if we exit by sending outpyt (op 4) we should reenter with sentoutput flag set to true to avoid sending again
        while self.data[self.i] != 99:
            ...
            if op == 1:
                ...
                if self.sentoutput == False:
                    v1 = self.GetValue(self.i+1, int(opcode[2]))
                    self.sentoutput = True
                    return v1
                else:
                    self.sentoutput = False
            elif op == 5:
